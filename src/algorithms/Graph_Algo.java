...
public class Graph_Algo implements graph_algorithms, Serializable {
	...
	private void Dijkstra(int src) {
		...
		while (!PrioQueue.isEmpty()) // while not all vertexes reached
		{
			...
			Collection<edge_data> edges = graph.getE(currNode.getKey()); // Extract minimum distance vertex from
			// prioQueue
			for (edge_data edge : edges) // Loop through all adjacent of u and do following for every destination
				// vertex.
			{
				node_data destNode = graph.getNode(edge.getDest()); // dest vertex
				if (destNode.getWeight() > currNode.getWeight() + edge.getWeight()) 
				{
					PrioQueue.remove(destNode);
					destNode.setWeight(currNode.getWeight() + edge.getWeight());
					destNode.setInfo(currNode.getKey() + "");
					destNode.setTag(currNode.getKey());
	...
	/**
	 * The function starts from the destination vertex, every vertex info is the
	 * prev vertex that we came from, that's how we backtrack finding the path.
	 **/
	@Override
	public List<node_data> shortestPath(int src, int dest) {
		...
		int key = dest;
		while (key != src && key != -1) {
			ans.add(graph.getNode(key));
			key = graph.getNode(key).getTag();
		}
		if (key == -1) {
			return null;
		}
	...
	private boolean isTherePath(int src, int dest) {
		// if(graph.getNode(src).getWeight() == Integer.MAX_VALUE ||
		// graph.getNode(dest).getWeight() == Integer.MAX_VALUE)
		// return false;
	...
	public List<node_data> TSP(List<Integer> targets) {
		...
		// targets.stream().distinct().collect(Collectors.toList());
